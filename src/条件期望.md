...
&emsp;&emsp;首先，我们使用测度论中的Radon Nikodym定理给出条件期望的严格定义。设$ (X,\mathcal F,P) $是一个概率空间，$ f $是它上面积分存在的随机变量。又设$ \mathcal G $是$ \mathcal F $的子$ \sigma $-域，即$  \mathcal G $是一个$ X $上的$ \sigma $域，而且$ \mathcal G \subset \mathcal F$。对每个$ A \in \mathcal G $，令$$ \varphi(A) = \int_{A} f dp $$  易见$ \varphi $是$ \mathcal G$上的符号测度，它和限制在$ \mathcal G $上的测度$ P $之间有关系$ \varphi \ll P$。因此，由Radon Nikodym定理知，存在$ (X,\mathcal F,P) $上$ a.s. $意义下唯一的可测函数$ E(f|\mathcal G) $，使对每个$ A \in \mathcal F $有$$ \varphi(A) = \int_{A} E(f|\mathcal G) dp $$(这个可测函数之所以记为$ E(f|\mathcal G) $，是因为它既与$ f $有关，又与$ \mathcal G $有关)。利用$ E(f|\mathcal G) $的性质，可以把条件期望公理化地定义如下：
> &emsp;&emsp;设$ f $为概率空间$ (X,\mathcal F,P) $上积分存在的随机变量。称$ E(f|\mathcal G) $为$ f $关于$  \mathcal F $的子$ \sigma $域$ \mathcal G $的条件期望，如果\
&emsp;&emsp;(1) $ E(f|\mathcal G) $是$ (X,\mathcal F,P) $上积分存在的可测函数；\
&emsp;&emsp;(2) 对任何$ A \in \mathcal G $，有$$ \int_{A} E(f|\mathcal G) dp = \int_{A} f dp $$

&emsp;&emsp;从通俗意义上理解，条件期望即是一种符号测度对一种测度的Radon Nikodym导数，而严格的Radon Nikodym导数定义如下：
> &emsp;&emsp;设$ \varphi $是测度空间$ (X,\mathcal F,\mu) $上的符号测度。如果存在$ a.e. $意义下唯一的可测函数使下式成立，则称$ f $为$ \varphi $对$ \mu $的Radon Nikodym导数$$ \rho(A) = \int_{A} f d \mu \quad,\quad \forall A \in \mathcal F $$
